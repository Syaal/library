...
public class BorrowService {
    ...
    public List<BorrowResponse> getAllBorrow(){
        ...
        return borrowBooks.stream().map(borrow ->
                mapper.map(borrow, BorrowResponse.class)).toList();
    }

    public BorrowBook getBorrowById(Integer id){
        ...
        if (borrowBookOptional.isPresent()){
            ...
            return mapper.map(borrowBook,BorrowBook.class);
        }else {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Borrow Id Not Found");
        }
    }
